      ...
      const findAndOpenPayslip = async () => {
        try {
          const { getPayslip: fetchPayslip } =
            await import("@/app/actions/payroll");
  ...
  const computePreview = async () => {
    ...
    try {
      ...
      for (const employeeId of selectedEmployees) {
        ...
        // Convert cutoff dates to timestamps
        const cutoffStartTs = new Date(cutoffStart).getTime();
        const cutoffEndTs = new Date(cutoffEnd).getTime();
        ...
        // Non-taxable allowance from employee compensation
  ...
  const handleSavePayrollRun = async () => {
    ...
    try {
      ...
      await updatePayrollRun({
        ...
        cutoffStart: new Date(editCutoffStart).getTime(),
        cutoffEnd: new Date(editCutoffEnd).getTime(),

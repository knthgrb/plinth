...
export function generatePayslipPinResetEmail(resetLink: string): {
  subject: string;
  html: string;
  text: string;
} {
  const subject = `Reset your payslip PIN - Plinth`;
  ...
  const html = `
    ...
    <body style="font-family: Arial, sans-serif; line-height: 1.6; color: #333; max-width: 600px; margin: 0 auto; padding: 20px;">
      ...
      <div style="background: #ffffff; padding: 30px; border: 1px solid #e5e7eb; border-top: none; border-radius: 0 0 8px 8px;">
        <h2 style="color: #1f2937; margin-top: 0;">Reset your payslip PIN</h2>
        <p>Hello,</p>
        <p>We received a request to reset your payslip PIN.</p>
        <p>Click the button below to set a new PIN:</p>
        <div style="text-align: center; margin: 30px 0;">
          <a href="${resetLink}" style="display: inline-block; background: #9333ea; color: white; padding: 12px 30px; text-decoration: none; border-radius: 6px; font-weight: bold;">Reset PIN</a>
        </div>
        <p style="color: #6b7280; font-size: 14px; margin-top: 30px;">If the button doesn't work, copy and paste this link into your browser:</p>
        <p style="color: #9333ea; font-size: 12px; word-break: break-all;">${resetLink}</p>
        <p style="color: #6b7280; font-size: 12px; margin-top: 30px;">This link will expire in 1 hour. If you didn't request a PIN reset, you can ignore this email.</p>
      </div>
      <div style="text-align: center; margin-top: 20px; color: #9ca3af; font-size: 12px;">
        <p>© ${new Date().getFullYear()} Plinth. All rights reserved.</p>
      </div>
  ...
  const text = `
Reset your payslip PIN

We received a request to reset your payslip PIN.

Reset your PIN by visiting this link:
${resetLink}

This link will expire in 1 hour. If you didn't request a PIN reset, you can ignore this email.

export function generateInvitationEmail(
  ...
  const subject = `Invitation to join ${organizationName} on Purple Pay`;
  ...
  const html = `
    ...
    <body style="font-family: Arial, sans-serif; line-height: 1.6; color: #333; max-width: 600px; margin: 0 auto; padding: 20px;">
      <div style="background: linear-gradient(135deg, #9333ea 0%, #7c3aed 100%); padding: 30px; text-align: center; border-radius: 8px 8px 0 0;">
        <h1 style="color: white; margin: 0;">Purple Pay</h1>
      </div>
      <div style="background: #ffffff; padding: 30px; border: 1px solid #e5e7eb; border-top: none; border-radius: 0 0 8px 8px;">
        <h2 style="color: #1f2937; margin-top: 0;">You've been invited!</h2>
        <p>Hello,</p>
        <p><strong>${inviterName}</strong> has invited you to join <strong>${organizationName}</strong> on Purple Pay as a <strong>${role}</strong>.</p>
      ...
      <div style="text-align: center; margin-top: 20px; color: #9ca3af; font-size: 12px;">
        <p>© ${new Date().getFullYear()} Purple Pay. All rights reserved.</p>
      </div>
    </body>
    </html>
  `;
...
${inviterName} has invited you to join ${organizationName} on Purple Pay as a ${role}.
...
© ${new Date().getFullYear()} Purple Pay. All rights reserved.
  `;
